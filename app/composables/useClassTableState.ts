...
export function useClassTableState<T extends { [key: string]: any }>(data: Ref<T[]>) {
  const currentPage = ref(1)
  const itemsPerPage = ref(10)
  const sort = ref<{ column: string, direction: 'asc' | 'desc' }>({ column: '', direction: 'asc' })
  const selectedRows = ref<T[]>([])

  const { searchTerm, filteredData, resetFilters, hasNotMainTeacherFilter } = useClassFilters(data)
